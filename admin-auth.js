...
function applyPermissionsUI(permission) {
  ...
  if (isViewer) {
    ...
    // Disable inputs & forms (excluding analytics filters & tabs so viewers can still interact with the dashboard filters and navigate tabs)
    const inputs = document.querySelectorAll("#admin input, #admin select, #admin button:not(#admin-logout)");
    inputs.forEach(el => {
      if (el.classList.contains("admin-tab-btn")) {
        return;
      }
      if (el.closest(".analytics-filters") && el.id !== "btn-reset-analytics" && el.id !== "btn-start-zero-analytics") {
        return;
      }
      el.disabled = true;
