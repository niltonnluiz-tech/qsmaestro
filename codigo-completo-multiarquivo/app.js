...
function renderBlockedDates() {
  ...
  document.querySelectorAll("[data-unblock-date]").forEach((button) => {
    button.addEventListener("click", () => {
      state.blockedDates = state.blockedDates.filter((item) => item.date !== button.dataset.unblockDate);
      persistBlockedDates();
      renderBlockedDates();
    });
  });
...
function persistContractContext() {
  ...
  writeStorage(storageKeys.contractContext, {
    lead: state.lead,
...
function bindAdminControls() {
  ...
  document.getElementById("add-admin-song")?.addEventListener("click", () => {
    ...
    state.midiFileName = "";
    renderRecommendation();
    alert("Música/MIDI adicionada ao catálogo do protótipo.");
  });
